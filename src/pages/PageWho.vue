<template>
  <div class="text-center q-pa-md">
    <div class="q-pa-md" />

    <!-- Darice -->

    <div class="row">
      <div class="col" />
      <div class="col-12 col-md-6">
        <div class="row">
          <div class="col-6">
            <q-img class="max-heght" src="../../assets/images/who/darice.png">
              <div class="absolute-bottom text-subtitle1 text-center">
                Darice
              </div>
            </q-img>
          </div>
          <div class="col-6 text-justify q-pl-lg">
            <h5>Darice Sousa</h5>
            <p>
              Formada no ensino médio integrado ao tectnico em informática pelo
              Instituto Federal do Piauí
            </p>
            <p>
              Graduando em Sistemas de informações pela Universidade Federal do
              Piauí
            </p>
            <a href="mailto:daricesousa@gmail.com"> daricesousa@gmail.com </a>
          </div>
        </div>
      </div>
      <div class="col" />
    </div>
    <div class="q-ma-lg" />

    <!-- Isdael -->

    <div class="row">
      <div class="col" />
      <div class="col-12 col-md-6">
        <div class="row">
          <div class="col-6 text-justify q-pr-lg">
            <h5>Isdael Rodrigues</h5>

            <p>Estudante de sitema de informação</p>

            <p>Focado atualmente em terminar os estudos.</p>
            <p>A linguagem que tenho maior afinidade e 'C'</p>
            <a href="mailto:isdael@gmail.com"> isdael@gmail.com </a>
          </div>
          <div class="col-6">
            <q-img class="max-heght" src="../../assets/images/who/isdael.png">
              <div class="absolute-bottom text-subtitle1 text-center">
                Isdael
              </div>
            </q-img>
          </div>
        </div>
      </div>
      <div class="col" />
    </div>
    <div class="q-ma-lg" />

    <!-- Paulo -->

    <div class="row">
      <div class="col" />
      <div class="col-12 col-md-6">
        <div class="row">
          <div class="col-6">
            <q-img class="max-heght" src="../../assets/images/who/paulo.png">
              <div class="absolute-bottom text-subtitle1 text-center">
                Paulo
              </div>
            </q-img>
          </div>
          <div class="col-6 text-justify q-pl-lg">
            <h5>Paulo Henrique</h5>
            <p>Estudante de sitema de informação</p>
            <p>
              Atualmente focado em mudar o mundo mesmo que em pequenas ações.
            </p>
            <p>A linguagem que tenho maior afinidade e 'Python'</p>
            <a href="mailto:paulo@gmail.com"> paulo@gmail.com </a>
          </div>
        </div>
      </div>
      <div class="col" />
    </div>
    <div class="q-ma-lg" />

    <!-- Weliton -->

    <div class="row">
      <div class="col" />
      <div class="col-12 col-md-6">
        <div class="row">
          <div class="col-6 text-justify q-pr-lg">
            <h5>Weliton Sousa</h5>
            <p>
              Formado no ensino médio integrado ao tectnico em informática pelo
              Instituto Federal do Piauí
            </p>
            <p>
              Graduando em Sistemas de informações pela Universidade Federal do
              Piauí
            </p>
            <ul>
              <li>Front end Developer;</li>
              <li>Back end Developer;</li>
              <li>Mobile Developer;</li>
            </ul>
            <a href="mailto:welitonubuntu@gmail.com">
              welitonubuntu@gmail.com
            </a>
            <br />
            <br />

            <a target="_blank" href="https://welitonsousa.vercel.app">
              <q-btn label="Web Site"></q-btn>
            </a>
          </div>
          <div class="col-6">
            <q-img class="max-heght" src="../../assets/images/who/weliton.png">
              <div class="absolute-bottom text-subtitle1 text-center">
                Weliton
              </div>
            </q-img>
          </div>
        </div>
      </div>
      <div class="col" />
    </div>
    <br />
    <br />
    <br />
    <br />
  </div>
</template>

<style scoped>
.max-heght {
  max-height: 30pc;
}
</style>